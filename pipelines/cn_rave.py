...
class RAVE(nn.Module):
    def __init__(self, device):
        ...
        self.cond_step_start = 0.0
        self.controlnet_guidance_start = 0.0
        self.controlnet_guidance_end = 1.0
        self.controlnet_conditioning_scale = 1.0
        self.is_shuffle = True
        self.give_control_inversion = True
    ...
    def pred_controlnet_sampling(self, current_sampling_percent, latent_model_input, t, text_embeddings, control_image):
        if (
            current_sampling_percent < self.controlnet_guidance_start or 
            current_sampling_percent > self.controlnet_guidance_end
        ):
    ...
    def denoising_step(self, latents, control_image, text_embeddings, t, guidance_scale, current_sampling_percent):
        ...
        noise_pred = self.pred_controlnet_sampling(
            current_sampling_percent, latent_model_input, t, text_embeddings, control_image)
    ...
    def preprocess_control_grid(self, image_pil):
        ...
        list_of_pils = [
            pu.pixel_perfect_process(np.array(frame_pil, dtype='uint8'), self.preprocess_name) 
    ...
    def batch_denoise(self, latents, control_image, indices, t, guidance_scale, current_sampling_percent):
        ...
        for idx in range(len(control_split)):
            txt_embed = torch.cat(
                [self.uncond_embeddings] * len(latents_split[idx]) + [self.cond_embeddings] * len(latents_split[idx])) 
            latents = self.denoising_step(
                latents_split[idx], control_split[idx], txt_embed, t, guidance_scale, current_sampling_percent)
    ...
    def __postprocess_inversion_input(self, latents_inverted, control_batch):
        latents_inverted = torch.cat([
            fu.unflatten_grid(
                torch.cat([
                    a for a in latents_inverted[i*self.grid_frame_number:(i+1)*self.grid_frame_number]
                ], dim=-1).unsqueeze(0), self.grid
            ) 
    ...
    def preprocess(
        ...
        self.sample_size = len(image_pil_list)
        ...
        self.num_inversion_step = num_inference_steps
        self.inverse_path = inverse_path

        img_batch, control_batch = self.process_image_batch(image_pil_list)
        ...
        self.scheduler.set_timesteps(self.num_inference_steps, device=self.device)
        self.inv_cond_embeddings, self.inv_uncond_embeddings = self.get_text_embeds(inversion_prompt, "")
        ...
        return latents_inverted, control_batch, indices
    ...
    def __call__(
        ...
        self.cond_embeddings, self.uncond_embeddings = self.get_text_embeds(
            positive_prompts, negative_prompts)
        latents_denoised, indices, controls = self.reverse_diffusion(
            latents_inverted, control_batch, guidance_scale, indices=indices)
